            ...
            <div className="space-y-1 pl-2">
              {group.items.map(item => (
                <label key={item} className="flex items-center gap-2 cursor-pointer select-none">
                  <input type="checkbox" checked={filters[group.key].includes(item)} onChange={e => setFilters(f => ({...f, [group.key]: e.target.checked ? [...f[group.key], item] : f[group.key].filter(i => i !== item)}))} className="form-checkbox accent-purple-500 w-4 h-4 rounded-full transition" />
